...
class WithdrawChart extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => _WithdrawChartState();
}

class _WithdrawChartState extends State<WithdrawChart> {
  final Color leftBarColor = Colors.red; //const Color(0xff53fdd7);
  ...
  void loadChartData() async {
    ...
    transaction.transactionsType == Constants.WITHDRAW_TYPE ||
        transaction.transactionsType == Constants.BANK_RECEIVE_TYPE ||
        transaction.transactionsType == Constants.PARTNER_RECEIVE_TYPE)
        .toList();
    ...
    double waveMoneyWithdrawAmount =
    waveMoneyTransactions.fold(0, (sum, item) => sum + item.amount);
    double kbzPayWithdrawAmount =
    kbzPayTransactions.fold(0, (sum, item) => sum + item.amount);
    double cbPayWithdrawAmount =
    cbPayTransactions.fold(0, (sum, item) => sum + item.amount);

    double trueMoneyWithdrawAmount =
    trueMoneyTransactions.fold(0, (sum, item) => sum + item.amount);

    final commissionData = [
      new SaleData('Wave Money', waveMoneyWithdrawAmount),
      new SaleData('Cb Pay', cbPayWithdrawAmount),
      new SaleData('True Money', trueMoneyWithdrawAmount),
      new SaleData('Kbz Pay', kbzPayWithdrawAmount),
    ];
    setState(() {
      saleData = [
        new charts.Series<SaleData, String>(
